...
tune_df    = df.loc[:'2020-12-31']                     # Step 1: CV tuning
val_df     = df.loc['2021-01-01':'2023-12-31']         # Step 2: expanding-window validation
test_df    = df.loc['2024-01-01':]                     # Step 3: final holdout
pretest_df = df.loc[:'2023-12-31']                     # for refitting before test evaluation
...
for ti, target in enumerate(ret_cols):
    ...
    predictors = [ar_term] + surp_cols + ['VIXCLS']
    ...
    # ── Per-fold standardisation helper (delegates to module-level _fold_dfs_fn) ─
    ...
    _XtX = X_tune_s.T @ X_tune_s      # computed once
    _XtY = X_tune_s.T @ Y_tune        # computed once
    _eye = np.eye(X_tune_s.shape[1])
    soln_ridge = np.array([
    ...
    for mname_c, vars_list_c in [
        ...
        if vars_list_c:
            idx_c = [predictors.index(v) for v in vars_list_c]
            pipe_c = Pipeline([('sc', StandardScaler()), ('lr', skl.LinearRegression())])
            pipe_c.fit(X_tune[:, idx_c], Y_tune)
            for v, c in zip(vars_list_c, pipe_c.named_steps['lr'].coef_):
                coef_rows.append({'Method': mname_c, 'Variable': v, 'Coefficient': c})
    for v, c in zip(predictors, grid_ridge.best_estimator_['ridge'].coef_):
        coef_rows.append({'Method': 'Ridge', 'Variable': v, 'Coefficient': c})
    for v, c in zip(predictors, grid_lasso.best_estimator_['lasso'].coef_):
        if abs(c) > 1e-8:
    ...
    def _build_subset(X_tr, Y_tr, spec):
        ...
        pipe = Pipeline([('sc', StandardScaler()), ('lr', skl.LinearRegression())])
        pipe.fit(X_tr[:, idx], Y_tr)
        return lambda X_te: pipe.predict(X_te[:, idx])[0]
