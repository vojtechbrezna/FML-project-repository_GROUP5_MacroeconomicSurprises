...
class SurpVixInteractor(BaseEstimator, TransformerMixin):
    ...
    If drop_vix=True, the standalone VIXCLS column is removed after interactions
    are appended, so VIX only enters the model through the interaction terms.
    Stateless: fit() is a no-op, so it is safe inside sklearn Pipelines."""
    def __init__(self, surp_idx, vix_idx, drop_vix=False):
        self.surp_idx = surp_idx   # list[int] — column indices for surp_ features
        self.vix_idx  = vix_idx    # int        — column index for VIXCLS
        self.drop_vix = drop_vix   # bool       — drop standalone VIX after interaction
    def fit(self, X, y=None):
        return self
    def transform(self, X):
        vix  = X[:, self.vix_idx : self.vix_idx + 1]  # (n, 1)
        surp = X[:, self.surp_idx]                     # (n, k)
        out  = np.hstack([X, surp * vix])              # (n, p + k)
        if self.drop_vix:
            out = np.delete(out, self.vix_idx, axis=1)
        return out


class VixPreserveScaler(BaseEstimator, TransformerMixin):
    """StandardScaler that treats VIXCLS as already daily-standardised (identity transform).
    All other columns are standardised using their announcement-day statistics."""
    def __init__(self, vix_idx):
        self.vix_idx = vix_idx
    def fit(self, X, y=None):
        self._sc = StandardScaler().fit(X)
        self._sc.mean_[self.vix_idx]  = 0.0
        self._sc.scale_[self.vix_idx] = 1.0
        self._sc.var_[self.vix_idx]   = 1.0
        return self
    def transform(self, X):
...
df = pd.read_csv("merged_announcement_panel_fedincl.csv", index_col=0)
df.index = pd.to_datetime(df.index, format='%Y-%m-%d')
df = df.sort_index()


# ── Pre-standardise VIXCLS using the full daily distribution (tune period) ───
_daily_vix = pd.read_csv("merged_daily_panel_fedincl.csv", index_col=0)
...
print(f"Predictors per target: AR(1) [if available] + {len(surp_cols)} surp + VIX (interaction only)")
...
def _fold_dfs_fn(tr_idx, te_idx, X_tune, X_tune_df, predictors,
                 interactor=None, all_cols=None, vix_idx=None):
    ...
    fs  = (VixPreserveScaler(vix_idx) if vix_idx is not None else StandardScaler()).fit(X_tune[tr_idx])
...
_CHECKPOINT = 'outcomes/_checkpoint_novix.pkl'
...
if os.path.exists(_CHECKPOINT):
    ...
    for _grp, _grp_cols in _HORIZONS.items():
        ...
        for ti, target in enumerate(_grp_cols):
            ...
            predictors = ([ar_term] if has_ar1 else []) + surp_cols + ['VIXCLS']
            ...
            scaler      = VixPreserveScaler(vix_idx_l).fit(X_tune)
            X_tune_s    = scaler.transform(X_tune)

            # ── Interaction terms: surp × vix on standardised data; VIX dropped ────────
            interactor  = SurpVixInteractor(surp_idx_l, vix_idx_l, drop_vix=True)
            inter_names = [f"{c}_x_vix" for c in surp_cols if c in predictors]
            # VIXCLS excluded from predictors_l — only interaction terms remain
            predictors_l  = [c for c in predictors if c != 'VIXCLS'] + inter_names
            X_tune_si     = interactor.transform(X_tune_s)  # standardised + interactions, VIX dropped
            ...
            # RIDGE — Pipeline: standardise → interact (drop VIX) → Ridge
            lambdas    = 10 ** np.linspace(8, -6, 100) / Y_tune.std()
            grid_ridge = skm.GridSearchCV(
                Pipeline([('sc', VixPreserveScaler(vix_idx_l)), ('ix', interactor), ('ridge', skl.Ridge())]),
            ...
            print("Ridge - Done")
            # LASSO — Pipeline: standardise → interact (drop VIX) → Lasso
            lasso_alphas = np.logspace(-6, 2, 100)
            grid_lasso   = skm.GridSearchCV(
                Pipeline([('sc', VixPreserveScaler(vix_idx_l)), ('ix', interactor),
            ...
            # PCR — Pipeline: standardise → interact (drop VIX) → PCA → LR
            max_comp = min(50, len(predictors_l), X_tune.shape[0] - 1)
            pipe_pcr = Pipeline([('sc', VixPreserveScaler(vix_idx_l)), ('ix', interactor),
            ...
            print("PCR - Done")
            # PLS — Pipeline: standardise → interact (drop VIX) → PLS
            ...
            for shap_model, shap_name in [(rf_tune_model, 'rf'), (lgb_tune_model, 'lgb')]:
                ...
                # VIX channel decomposition via SHAP interaction values
                shap_inter = shap_explainer.shap_interaction_values(X_tune)  # (n, p, p)
                vix_main      = shap_inter[:, vix_idx_l, vix_idx_l]          # main effect (diagonal)
                vix_inter_row = shap_inter[:, vix_idx_l, :]                  # interactions with all features
                inter_df = pd.DataFrame(
                    vix_inter_row, columns=predictors, index=X_tune_df.index,
                )
                inter_df.insert(0, 'vix_main_effect', vix_main)
                inter_df.to_csv(f'outcomes/shap_vix_decomp_{shap_name}_{target}.csv')
                decomp_summary = pd.DataFrame({'mean_abs_shap': inter_df.abs().mean()})
                decomp_summary.to_csv(f'outcomes/shap_vix_decomp_summary_{shap_name}_{target}.csv')
                print(f"\n  [{shap_name.upper()}] VIX SHAP decomposition ({target}):")
                print(decomp_summary.sort_values('mean_abs_shap', ascending=False)
                      .head(10).to_string(float_format='%.6f'))
            ...
            def _build_ar1(X_tr, Y_tr, spec):
                # AR(1) uses raw predictor column 0 — no interactions needed
                pipe = Pipeline([('sc', StandardScaler()), ('lr', skl.LinearRegression())])
                pipe.fit(X_tr[:, [0]], Y_tr)
                return lambda X_te: pipe.predict(X_te[:, [0]])[0]

            def _build_subset(X_tr, Y_tr, spec):
                # standardise → interact (drop VIX) → select columns → OLS
                vl = spec['vars_list']
                if not vl:
                    mean_val = float(Y_tr.mean())
                    return lambda X_te: mean_val
                idx = [_pred_list.index(v) for v in vl]
                sc = VixPreserveScaler(vix_idx_l).fit(X_tr)
            ...
            def _build_ridge(X_tr, Y_tr, spec):
                pipe = Pipeline([('sc', VixPreserveScaler(vix_idx_l)), ('ix', interactor),
            ...
            methods_val = []
            if has_ar1:
                methods_val.append(('AR(1)', _build_ar1, {}))
            methods_val += [
            ...
            methods_test = []
            if has_ar1:
                methods_test.append(('AR(1)', _build_ar1, {}))
            methods_test += [
                ('Fwd (Cp)',  _build_subset, {'vars_list': fwd_cp_vars}),
                ('Fwd (CV)',  _build_subset, {'vars_list': fwd_cv_vars}),
                ('Bwd (Cp)',  _build_subset, {'vars_list': bwd_cp_vars}),
        ...
        _summary_plots = [
            (fig_fwd,   f'outcomes/fwd_stepwise_summary_{_grp}.png'),
            (fig_bwd,   f'outcomes/bwd_stepwise_summary_{_grp}.png'),
            (fig_rpath, f'outcomes/ridge_path_summary_{_grp}.png'),
            (fig_rcv,   f'outcomes/ridge_cv_summary_{_grp}.png'),
            (fig_lpath, f'outcomes/lasso_path_summary_{_grp}.png'),
            (fig_lcv,   f'outcomes/lasso_cv_summary_{_grp}.png'),
            (fig_dim,   f'outcomes/pcr_pls_summary_{_grp}.png'),
            (fig_oos,   f'outcomes/oos_r2_summary_{_grp}.png'),
        ]
...
summary_cv.to_csv('outcomes/model_selection_results_cv_novix.csv')
summary_val_mse.to_csv('outcomes/model_selection_results_val_mse_novix.csv')
summary_val_r2.to_csv('outcomes/model_selection_results_val_r2_novix.csv')
summary_val_ic.to_csv('outcomes/model_selection_results_val_ic_novix.csv')
summary_oos_mse.to_csv('outcomes/model_selection_results_oos_novix.csv')
summary_oos_r2.to_csv('outcomes/model_selection_results_oos_r2_novix.csv')
summary_oos_ic.to_csv('outcomes/model_selection_results_oos_ic_novix.csv')
print("\nSaved: outcomes/model_selection_results_{cv, val_mse, val_r2, val_ic, oos, oos_r2, oos_ic}_novix.csv")
...
).to_csv('outcomes/insample_stats_all_novix.csv', index=False)
print("Saved: outcomes/insample_stats_all_novix.csv")

# Selected variables + coefficients for all targets → single CSV
if all_coefs:
    pd.concat(
        [df.assign(target=t) for t, df in all_coefs.items()]
    ).to_csv('outcomes/selected_vars_coefs_novix.csv', index=False)
    print("Saved: outcomes/selected_vars_coefs_novix.csv")
...
pd.DataFrame(_pred_val).to_csv('outcomes/y_pred_val_novix.csv')
pd.DataFrame(_pred_test).to_csv('outcomes/y_pred_test_novix.csv')
print("Saved: outcomes/y_pred_val_novix.csv")
print("Saved: outcomes/y_pred_test_novix.csv")
