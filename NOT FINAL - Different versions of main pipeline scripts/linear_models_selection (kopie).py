...
df = pd.read_csv("merged_announcement_panel.csv", index_col=0)
...
def _fwd_fold_worker(tr_idx, te_idx, X_tune, X_tune_df, predictors,
    ...
    Xf_tr, Xf_te = _fold_dfs_fn(tr_idx, te_idx, X_tune, X_tune_df, predictors,
                                  interactor=interactor, all_cols=all_cols)
...
if os.path.exists(_CHECKPOINT):
    ...
    for _grp, _grp_cols in _HORIZONS.items():
        ...
        for ti, target in enumerate(_grp_cols):
            ...
            fwd_cp_cv_mse = _cv_mse_of_vars(fwd_cp_vars, X_tune_si, predictors_l, Y_tune, tscv)
            ...
            bwd_cp_cv_mse = _cv_mse_of_vars(bwd_cp_vars, X_tune_si, predictors_l, Y_tune, tscv)
            ...
            grid_ridge = skm.GridSearchCV(
                Pipeline([('sc', StandardScaler()), ('ix', interactor), ('ridge', skl.Ridge())]),
            ...
            grid_lasso   = skm.GridSearchCV(
                Pipeline([('sc', StandardScaler()), ('ix', interactor),
            ...
            pipe_pcr = Pipeline([('sc', StandardScaler()), ('ix', interactor),
            ...
            def _build_ridge(X_tr, Y_tr, spec):
                pipe = Pipeline([('sc', StandardScaler()), ('ix', interactor),
