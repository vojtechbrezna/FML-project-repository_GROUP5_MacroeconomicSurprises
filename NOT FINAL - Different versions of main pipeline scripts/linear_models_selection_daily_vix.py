...
df = pd.read_csv("merged_announcement_panel_fedincl_nextday.csv", index_col=0)
df.index = pd.to_datetime(df.index, format='%Y-%m-%d')
df = df.sort_index()


# ── Pre-standardise VIXCLS using the full daily distribution (tune period) ───
_daily_vix = pd.read_csv("merged_daily_panel_fedincl_nextday.csv", index_col=0)
...
print(f"Predictors per target: AR(1) [if available] + {len(surp_cols)} surp + 1 VIX")
...
_CHECKPOINT = 'outcomes/_checkpoint.pkl'
