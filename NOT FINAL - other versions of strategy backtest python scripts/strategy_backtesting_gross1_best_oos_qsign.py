"""
strategy_backtesting_gross1_best_oos_qsign.py
=============================================

Variant of strategy_backtesting_gross1_best_oos.py where the quartile strategy
is enhanced with a sign filter:
  - Long  only when predicted return is in the upper quartile AND pred > 0
  - Short only when predicted return is in the lower quartile AND pred < 0
"""
...
OUTPUT_DIR               = "strategy_outcomes/quartile_sign_filter"
...
#   sign     – original sign strategy (unchanged)
#   quartile – upper quartile AND pred>0 → long; lower quartile AND pred<0 → short
...
for _, row in selected.iterrows():
    ...
    base_ret = log_to_simple(panel_test[base_col].values)
    ...
    for strat in STRATEGIES:
        ...
        for sub in SUBSTRATEGIES:
            ...
            pnl_num_arr = np.zeros(len(panel_dates), dtype=float)
            gross_arr   = np.zeros(len(panel_dates), dtype=float)
            pos_dict: dict = {}
            ...
            for ann_date, pred in preds.items():
                ...
                else:  # quartile – upper quartile AND positive → long
                       #            lower quartile AND negative → short
                    if sub == "full":
                        pos = (1.0  if (pred > q_high and pred > 0) else
                               -1.0 if (pred < q_low  and pred < 0) else 0.0)
                    elif sub == "long":
                        pos = 1.0  if (pred > q_high and pred > 0) else 0.0
                    else:
                        pos = -1.0 if (pred < q_low  and pred < 0) else 0.0
                # ────────────────────────────────────────────────────────────
            ...
            daily_ret_arr = np.divide(pnl_num_arr, gross_arr,
                                      out=np.zeros_like(pnl_num_arr),
                                      where=gross_arr > 0)

            daily_pnl[label] = pd.Series(daily_ret_arr, index=panel_dates, dtype=float)
...
results_sign_df.to_csv(os.path.join(OUTPUT_DIR, "strategy1_sign_results.csv"),         index=False)
results_quartile_df.to_csv(os.path.join(OUTPUT_DIR, "strategy2_quartile_results.csv"), index=False)
...
for asset in base_assets:
    ...
    for label, pnl in daily_pnl.items():
        ...
        try:
            target_part = col_name.split("_", 1)[1]
        except IndexError:
            continue
