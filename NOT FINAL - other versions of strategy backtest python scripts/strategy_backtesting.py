...
=======================
Backtests trading strategies driven by ML model predictions.

Strategies
----------
1. Sign strategy   – long if pred > 0, short if pred < 0
2. Quartile strategy – long if pred > Q(100-p), short if pred < Q(p), flat otherwise
   Quartile thresholds are set from the VALIDATION set (no look-ahead bias).

Each strategy has three sub-strategies: full (long+short), long-only, short-only.

Daily P&L is constructed precisely: for a position entered at announcement date t
with horizon H, the position is spread over the H consecutive trading days starting
at t, earning the 1-day base return each day. Multiple overlapping positions are
additive.

Benchmark: Buy-and-Hold for each unique base asset using full daily returns.

Outputs (saved to OUTPUT_DIR)
-----------------------------
- filtered_models_list.csv
- strategy1_sign_results.csv
- strategy2_quartile_results.csv
- benchmark_results.csv
- daily_returns.csv          ← daily P&L for every strategy column
- plots/{asset}_cumret.png
"""
...
def compute_metrics(daily_r: pd.Series,
    ...
    Parameters
    ----------
    daily_r       : daily P&L series over full test window (0 on non-active days)
...
selected.to_csv(os.path.join(OUTPUT_DIR, "filtered_models_list.csv"), index=False)
...
# daily_pnl  : label → daily P&L Series over panel_dates
...
for _, row in selected.iterrows():
    ...
    # base 1-day return array for fast indexing
    base_ret = panel_test[base_col].values    # aligned to panel_dates
    ...
    for strat in STRATEGIES:
        ...
        for sub in SUBSTRATEGIES:
            ...
            pnl_arr = np.zeros(len(panel_dates), dtype=float)
            pos_dict: dict = {}
...
for _, row in selected.iterrows():
    ...
    if target in panel_test.columns:
        actual_cumH = panel_test.loc[ann_dates, target].dropna()
    else:
    ...
    for strat in STRATEGIES:
        for sub in SUBSTRATEGIES:
            label = f"{col}|{strat}|{sub}"
            if label not in daily_pnl:
                continue

            metrics = compute_metrics(
                daily_r      = daily_pnl[label],
                ann_positions= ann_pos[label],
                actual_cumH  = actual_cumH,
                rf_series    = rf_test,
                oos_r2       = oos_r2_val,
            )
            record = {"model": model, "target": target, "horizon": H,
                      "substrategy": sub, **metrics}
...
results_sign_df.to_csv(os.path.join(OUTPUT_DIR, "strategy1_sign_results.csv"),
                       index=False)
results_quartile_df.to_csv(os.path.join(OUTPUT_DIR, "strategy2_quartile_results.csv"),
                           index=False)
...
for asset in base_assets:
    if asset not in panel_test.columns:
        continue
    daily_r  = panel_test[asset].dropna()
...
bh_df.to_csv(os.path.join(OUTPUT_DIR, "benchmark_results.csv"), index=False)
...
for asset in base_assets:
    ...
    ax.set_title(f"Cumulative Returns — {asset}", fontsize=13)
    ax.set_xlabel("Date")
    ax.set_ylabel("Gross cumulative return  (1.0 = start)")
    ax.legend(fontsize=6, ncol=3, loc="upper left",
              framealpha=0.6, bbox_to_anchor=(0, 1))
    ax.grid(True, alpha=0.25)
    fig.tight_layout()

    plot_path = os.path.join(OUTPUT_DIR, "plots", f"{asset}_cumret.png")
